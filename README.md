...
- Sign-in with three roles — viewer, user, administrator — and TOTP as
  ...
  to be plain HTTP: see [Users and sign-in](#users-and-sign-in).
...
| v0.7.4 ✓| Sign-in, three roles and a second factor (TOTP) |
| v0.7.5  | HTTPS (TLS of its own or behind a reverse proxy) and passkeys (WebAuthn) |
| later   | Commands on the server from the config, for administrators |
...
In the browser, **Users** in the header does the same for an
...
administrator. Everyone's own page (the name in the header) changes the
password and binds TOTP.
...
MoonLan v0.7.4 speaks plain HTTP. Signing in keeps out somebody at a
...
v0.7.5 closes it: HTTPS — TLS of its own or behind a reverse proxy —
...
│   ├── auth.py             # scrypt passwords, TOTP, recovery codes, tokens
│   ├── access.py           # the rights table: every route and the role it needs
│   ├── signin.py           # sessions, the guard on every request, sign-in, accounts API
│   ├── users.py            # python -m moonlan.users: accounts from the server's shell
...
| GET    | `/api/health`     | `{"status": "ok", "version"}` without sign-in, for monitoring from outside |
| GET    | `/api/auth/me`    | Who this browser is signed in as, or that sign-in is off; 401 when nobody |
| POST   | `/api/auth/login` | `{"name", "password"}` (JSON); with TOTP on, answers `code_required` and a ticket |
| POST   | `/api/auth/totp`  | `{"ticket", "code"}` — a TOTP or a recovery code; completes the sign-in |
| POST   | `/api/auth/logout` | Ends this session |
| POST   | `/api/auth/password` | `{"current", "new"}` — one's own password |
| POST   | `/api/auth/totp/setup` | A new secret to scan; binds nothing |
| POST   | `/api/auth/totp/confirm` | `{"code", "password"}` — binds it; answers with the recovery codes |
| GET, POST | `/api/users`   | Accounts (administrators); a new one gets a temporary password |
| PATCH, DELETE | `/api/users/{name}` | Role, disabled; delete |
| POST   | `/api/users/{name}/password`, `/reset-totp`, `/unlock`, `/logout` | A new temporary password, remove TOTP, lift a lock, sign out everywhere |

With sign-in on, every route needs the role written for it in
`moonlan/access.py` (401 without a session, 403 with the role needed),
and changing requests must carry the map's own `Origin`.
