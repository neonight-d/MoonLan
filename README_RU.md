...
- Loop Detection по приватным MIB производителей: профили моделей,
  опознаваемые по `sysObjectID` (встроены D-Link DGS-1210-26 Rev.F1,
  DES-1210-28/ME и DES-3526, свои добавляются в
  `loop_detection.profiles`); опрос идёт в цикле счётчиков, а не в
  десятиминутном скане. `loop_detected` — critical, с сырым значением
...
- Мониторинг трафика и ошибок портов: лёгкий цикл опроса счётчиков
  ...
  данных: остальные собираются и показываются как обычно. Панель «Порты»
...
известной ветке одним GET: ветка одного производителя — не то, что
реализует другой, поэтому ответ по ней и есть опознание. `diag --loop`
печатает, каким из двух способов профиль подошёл, и прошивка с новым
`sysObjectID` не перестаёт наблюдаться молча.
