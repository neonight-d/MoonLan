...
- Стейтфул-тревоги: host_down (3 пропущенных ping, только для хостов
  ...
  с гистерезисом в два цикла), port_hosts_down (critical: несколько
  хостов одного порта замолчали разом — одна тревога вместо пачки),
  lag_degraded (упал член LAG), new_mac. Панель «Тревоги» показывает
...
thresholds:
  ...
  mass_down_hosts: 3             # port_hosts_down: хостов одного порта,
