...
- Коммутаторы, которых никто не опрашивает, — по именам: LLDP-сосед
  ...
  именованным узлом с моделью и кликабельным управляющим адресом,
  заменяя на этом порту безымянный «Коммутатор без SNMP» и забирая
  себе его устройства. Новая тревога `unmanaged_bridge_detected`
  поднимается, когда такой мост объявляется за портом, не являющимся
  магистральным; `known_bridges` подавляет её для тех, кому там место.
...
| v0.6.1 | Loop Detection по приватным MIB D-Link/HPE |
...
- **Мосты получают имена.** Сосед с capability `bridge`, чей chassis
  ...
  именем, моделью и управляющим адресом вместо безымянного
  «Коммутатора без SNMP» — или вместо ничего, если за ним было слишком
  мало устройств, чтобы сработал `unmanaged_threshold`.

Две оговорки встроены в код, обе выучены на практике:

**Пересылка LLDP-кадров.** Часть коммутаторов умеет ретранслировать
чужие LLDP-кадры (`LLDP Forward Message` на D-Link DES-1210). Сосед
тогда виден на порту, к которому не подключён, а на одном порту
оказываются два устройства. MoonLan это распознаёт — несколько соседей
на одном локальном порту или один chassis id на нескольких портах
того же коммутатора, — помечает порт в панели портов с пояснением и
никак не использует эти данные для построения связей. Если вы видите
пометку, выключите настройку: пользы от неё нет и вам.

**Отсутствие capability ничего не доказывает.** System Name,
Description и Capabilities — необязательные TLV LLDP, и часть устройств
поставляется с выключенными (у D-Link DES-3526 именно так). Такой сосед
показывается как неопознанное устройство LLDP: на карте есть, тревог не
поднимает. Отсутствие признака `bridge` не доказывает, что устройство
не коммутатор.
