...
- Читаемая карта любого размера: офлайн-устройства одного порта
  рисуются одним узлом «Офлайн · N», а не облаком серых точек вокруг
  каждого коммутатора (`offline_group_threshold`); крупные группы по
  умолчанию свёрнуты (`offline_group_collapse_at`), в карточке —
  список устройств с временем последнего появления. Подпись
  выделенного узла получает скруглённую подложку и читается поверх
  рёбер и соседей, а физика раскладки останавливается после
  стабилизации — сотни узлов не грузят процессор.
...
offline_group_threshold: 2   # офлайн-устройства одного порта рисуются
                             # одним узлом «Офлайн · N»
offline_group_collapse_at: 5 # группы крупнее — свёрнуты по умолчанию
...
| Метод | Путь              | Описание |
|-------|-------------------|----------|
| GET   | `/api/topology`   | Текущая топология: узлы, связи (порты, LACP, текущая загрузка), хосты (IP, имя, ping, VLAN, `stale`), `unlocated` (известные устройства вне портов), `pseudo_switches`, `vlan_names` |
